...
class Config(object):
    ...
    def __init__(self, dataset, embedding):
        ...
        self.embedding_pretrained = torch.tensor(
            np.load(dataset + '/data/' + embedding + '.npz')["embeddings"].astype('float32'))\
        ...
        self.warmup_ratio = 0.1 # 学习率预热比例
        self.clip = 0.5 # 梯度裁剪阈值
        self.embed = 512 # 词向量维度
        self.dim_model = 512
        self.hidden = 1024
        self.last_hidden = 512
        self.num_head = 32    # 多头的数量
        self.num_encoder = 8 # encoder的层数
        self.num_layers = 1                                             # lstm层数
        self.eval_interval = 100 # 每隔多少个step输出一次验证集上的效果
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.postion_embedding = Positional_Encoding(
            config.dim_model, config.pad_size,  # 修改为 config.dim_model
        ...
        # 全连接
        self.fc1 = nn.Linear(
            config.dim_model, config.num_classes)  # 修改为 config.dim_model
        self.global_avg_pool = nn.AdaptiveAvgPool1d(1)  # 添加全局平均池化层
    ...
    def forward(self, x):
        ...
        for encoder in self.encoders:
            out = encoder(out)
        # encoder的输出
        out = out.permute(0, 2, 1)  # 将维度转换为 (batch_size, dim_model, seq_len)
        out = self.global_avg_pool(out).squeeze(-1)  # 应用全局平均池化
        # 全连接
        out = self.fc1(out)
        return out

    def get_sentence_vectors(self, x):
        """获取句子的向量表示（encoder输出）"""
        out = self.embedding(x[0])
        out = self.postion_embedding(out)
        for encoder in self.encoders:
            out = encoder(out)
        # 返回所有token的表示（包括[CLS]等特殊token）
        return out
...
class Positional_Encoding(nn.Module):
    def __init__(self, dim_model, pad_size, dropout, device):  # 修改为 dim_model
        super(Positional_Encoding, self).__init__()
        self.device = device
        self.pe = torch.tensor([[pos /
                                 (10000.0 **
                                  (i // 2 * 2.0 / dim_model))  # 修改为 dim_model
                                 for i in range(dim_model)]  # 修改为 dim_model
    ...
    def forward(self, x):
        # 构建残差网络
        out = x + self.pe.to(self.device)  # 修改为 self.pe
        # 放弃神经元，防止过拟合
